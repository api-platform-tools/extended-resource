...
namespace ApiPlatformTools\Attribute;
...
class ExtendedApiResource extends ApiResource
{
    public function __construct(
        ...
        try {
            ...
            if (false === $parent) {
                throw new InvalidArgumentException(message: sprintf('%s must only be used with parent class, no parent found on %s', __CLASS__, $callerClass));
            }

            $resource = $parent->getAttributes(name: ApiResource::class);
            if ([] === $resource) {
                throw new InvalidArgumentException(message: sprintf('%s must only be used to extend %s attribute, no such attrbute found on %s', __CLASS__, ApiResource::class, $callerClass));
            }
